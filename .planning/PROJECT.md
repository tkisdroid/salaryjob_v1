...
## Current Milestone: v1.1 Ship-Ready — Gap Fill & Full QA Sweep

**Goal:** v1.0 close 시점에 남은 모든 gap을 메우고 실 사용자 플로우가 전 경로에서 완벽하게 동작하도록 검증·수정한다.

**Target features:**
- Phase 6 DB 마이그레이션 적용 + 스키마 일치 검증 (runtime blocker 해제)
- HUMAN-UAT 13 시나리오 실행 (Phase 4: 5 + Phase 5: 3 + Phase 6: 5) — MOCK-LOG 정책 적용
- UI/UX QA Full Sweep — Worker+Business+Admin 전 55 routes × Desktop+Mobile 375px (버튼 중복/오동작, 디자인 일관성, 빈 상태/에러, 네비게이션, 라우트 누락)
- Phase 1 legacy 잔재 정리 (/my/schedule 로컬 MOCK → 실 DB wiring, stale Clerk TODO 제거, CLOVA_OCR_SECRET 프로비저닝 가이드)

**Key context:**
- **MOCK + 기록 정책**: 외부 API 키(CLOVA, 결제 등) 없는 시나리오는 MOCK 경로로 검증 후 `.planning/phases/*/MOCK-LOG.md`에 "무엇을 mock했는지 + 실 키 확보 후 재검증 필요" 기록. 실 키 확보 후 재UAT는 v1.2 또는 v2로 이월.
- **사용자 관점 검증**: "탐색→지원→확정→근무→리뷰→정산" 루프가 코드가 아닌 브라우저에서 1분 이내 완료되어야 close 가능.
- **Phase 번호 연속**: v1.0이 Phase 6에서 종료 → v1.1은 Phase 7부터 시작.
...
### Active (v1.1 Ship-Ready scope)

- [ ] Phase 6 DB 마이그레이션 적용 — `commissionRate` / `businessRegImageUrl` / `Application.commissionRate|commissionAmount|netEarnings` / `business-reg-docs` storage bucket + admin seed. Runtime blocker until applied.
- [ ] HUMAN-UAT 13 시나리오 실행 — Phase 4 (5: Kakao Maps / Web Push / Realtime 2-tab / QR 스캔 / Geofence GPS), Phase 5 (3: 1분 루프, 375px, 리뷰 UX taste), Phase 6 (5: admin 검색, biz verify, 이미지 게이트, OCR 라운드트립, admin detail).
- [ ] UI/UX QA Full Sweep — Worker+Business+Admin 전 55 routes × Desktop+Mobile 375px. 버튼 중복/오동작/비활성 상태, 디자인 일관성, 빈 상태, 에러 토스트, 네비게이션 누락 체크리스트화.
...
- [ ] 인프라 보완: `CLOVA_OCR_SECRET` 프로비저닝 가이드 문서화, signed URL TTL 실측 (또는 MOCK-LOG 기록 후 defer).
- [ ] MOCK-LOG 정책: 외부 키 없는 시나리오는 MOCK 경로로 검증 후 `.planning/phases/*/MOCK-LOG.md`에 기록 (재검증은 v1.2/v2).
...
---
*Last updated: 2026-04-15 — v1.1 Ship-Ready milestone started (gap fill + HUMAN-UAT 13 + UI/UX QA full sweep + Phase 1 legacy cleanup). v1.0 shipped with `tech_debt` audit.*
