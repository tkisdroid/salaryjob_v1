 ...
 *
 * Phase 9 Plan 02 Task 02-02 — Business persona 4-bucket checklist.
 *
 * Clone of worker-checklist.spec.ts with PERSONA=biz + biz-specific
 ...
 *
 * Covers QA-02 (Business routes all buckets) and QA-05b (button-dup) for the
 * 20 biz routes in tests/review/routes/manifest.ts seedAs='biz'.
 */
...
test.describe(`@phase9-${PERSONA}`, () => {
  ...
  test.afterAll(() => {
    ...
    // Literal shard prefix `phase9-biz-` so plan verification grep matches
    // without depending on template-literal expansion.
    writeFileSync(
      join(".review", `phase9-biz-${projectTag}.json`),
