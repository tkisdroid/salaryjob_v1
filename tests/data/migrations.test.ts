// REQ: DATA-03 — 6 tables exist after migration; RLS enabled on user/profile tables; disabled on jobs/applications/reviews
...
describe.skipIf(skipIfNoSupabase())('DATA-03 migrations', () => {
  ...
  it('RLS is enabled on users, worker_profiles, business_profiles', async () => {
    const { Client } = await import('pg');
    const client = new Client({ connectionString: process.env.DIRECT_URL });
    await client.connect();
    const res = await client.query(
